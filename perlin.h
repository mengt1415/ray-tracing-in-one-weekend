...
public:
	double noise(const vec3& p) const
	{
		...
		*/
		...
		//to make it smooth,linearly interpolate
		/*double u = p.x() - floor(p.x());
		double v = p.y() - floor(p.y());
		double w = p.z() - floor(p.z());
		int i = floor(p.x());
...
static vec3* perlin_generate()
{
	vec3* p = new vec3[256];
	for (int i = 0; i < 256; ++i)
		p[i] = unit_vector(vec3(-1+2*my_drand48(),-1+2*my_drand48(),-1+2*my_drand48()));
	return p;
}
